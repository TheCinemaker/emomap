...
const EMOTION_RGB = {
  happy: [0, 255, 0],     // #00ff00 - Euphoria
  bored: [160, 160, 160], // #a0a0a0 - Null
  stressed: [255, 0, 0],     // #ff0000 - Panic
  tired: [255, 255, 0],   // #ffff00 - Low Bat
  motivated: [0, 204, 255],   // #00ccff - Focus
  love: [255, 0, 255],   // #ff00ff - Sync
  hype: [191, 0, 255],   // #bf00ff - Overdrive
};
...
export function useMoodGrid(bounds, sessionId, mapZoom, extraData = []) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // Merge real data with extraData (demo data)
        // Filter extraData to be within bounds to avoid unnecessary processing
        const relevantExtraData = extraData.filter(d =>
  ...
  }, [JSON.stringify(bounds), mapZoom, extraData.length]); // ✅ Zoom szint és extraData figyelése
